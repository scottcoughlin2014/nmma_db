...
def generate_conf(service):
    if service not in config["nmma"]["supervisord"].keys():
        raise ValueError(
            f"{service} service def not found in config['nmma']['supervisord']"
        )

    sc = ""
    for k in config["nmma"]["supervisord"][service].keys():
        sc += f"[{k}]\n"
        for kk, vv in config["nmma"]["supervisord"][service][k].items():
            sc += f"{kk} = {vv}\n"
